...
export function checkContentSafety(content: string): SafetyCheckResult {
  ...
  // 檢查時代錯置
  const anachronisticKeywords = SAFETY_KEYWORDS.anachronistic.filter(
  ...
  // 檢查不當美化
  const glorificationKeywords =
    SAFETY_KEYWORDS.inappropriate_glorification.filter((keyword) =>
      lowerContent.includes(keyword.toLowerCase())
    );
  ...
  return {
    isSafe:
      issues.length === 0 || issues.every((issue) => issue.severity === "low"),
    issues,
    alternativePrompt:
      issues.length > 0
        ? generateAlternativePrompt(content, issues)
        : undefined,
  };
}
...
export function checkResponseSafety(response: string): SafetyCheckResult {
  ...
  // 檢查是否有過度美化暴力的描述
  const violenceGlorificationPattern =
    /(偉大的征服|英明的決策.*殺|智慧的鎮壓|必要的清除)/gi;
